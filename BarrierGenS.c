...
typedef struct {
	TYPE CALIGN group;
	volatile unsigned short int CALIGN count;
	volatile unsigned short int CALIGN generation;
	CBDECL();
} Barrier;
...
static inline bool block( Barrier * b ) {
	...
	unsigned short int mygen = b->generation;
...
// Local Variables: //
// compile-command: "gcc -Wall -Wextra -std=gnu11 -O3 -DNDEBUG -fno-reorder-functions -DPIN -DAlgorithm=BarrierGenS Harness.c -lpthread -lm -D`hostname` -DCFMT" //
// End: //
