	...
	for ( int r = 0; r < RUNS; r += 1 ) {
		...
		for ( entry = 0; stop == 0; entry += 1 ) {
			...
			for ( ;; ) {
			  ...
			  if ( last != id ) {
					await( intents[other] == DontWantIn );
					break;
				} // if
...
// Local Variables: //
// tab-width: 4 //
// compile-command: "gcc -Wall -Wextra -std=gnu11 -O3 -DNDEBUG -fno-reorder-functions -DPIN -DAlgorithm=DekkerB Harness.c -lpthread -lm -D`hostname` -DCFMT -DCNT=0" //
// End: //
